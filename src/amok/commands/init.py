...
DEFAULT_WORKSPACE_DIR = Path("/var/tmp/amok")
...
class Init_command(Command, name = "init"):
	...
	def setup(subparser: argparse.ArgumentParser) -> None:
		subparser.add_argument(
			"-t", "--template",
			choices = ["fedora"], #, "centos", "almalinux"],
			default = None,
	...
	def execute(self) -> None:
		if self.args.workspace is None or not self.args.workspace.is_absolute():
			try:
				os.mkdir(DEFAULT_WORKSPACE_DIR)
				logging.debug("created directory at %s", DEFAULT_WORKSPACE_DIR)
			except FileExistsError:
				pass
		if self.args.workspace is None:
			self.args.workspace = Path(tempfile.mkdtemp(dir = DEFAULT_WORKSPACE_DIR))
		elif not self.args.workspace.is_absolute():
			self.args.workspace = DEFAULT_WORKSPACE_DIR / self.args.workspace
		...
		if self.args.workspace.is_dir() and not self.args.force:
			...
			template = templates.get(self.args.template, Template())
